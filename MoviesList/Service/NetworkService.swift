...
class NetworkService {
    ...
    func getMovies(page: Int = 1, completion: @escaping (_ data: Result?) -> ()) {
        let url = URL(string: "\(urlAPI)/movie/now_playing?api_key=\(apiKey)&language=ru&page=\(page)")
        let request = URLRequest(url: url!, cachePolicy: .reloadIgnoringLocalAndRemoteCacheData, timeoutInterval: 10)
        let task1 = URLSession.shared.dataTask(with: request) { (data, responce, _) in
    ...
    func searchMovies(page: Int = 1, query: String, completion: @escaping (_ movies: Result?) -> ()) {
        ...
        let url = URL(string: "\(urlAPI)/search/movie?language=ru&api_key=\(apiKey)&query=\(queryUrl)&page=\(page)")
        let task = URLSession.shared.dataTask(with: url!) { (data, responce, _) in
            DispatchQueue.main.async(execute: { () -> Void in
                if let httpResponse = responce as? HTTPURLResponse, httpResponse.statusCode < 299 {
                    guard let data = data else { return }
                    let moviesJSON = try! JSONDecoder().decode(Result.self, from: data)
